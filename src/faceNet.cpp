...
FaceNetClassifier::FaceNetClassifier(Logger gLogger, DataType dtype, const string uffFile, const string engineFile, int batchSize, bool serializeEngine,
...
void FaceNetClassifier::featureMatching(cv::Mat &image)
{
    ...
    for (int i = 0; i < (m_embeddings.size() / 128); i++)
    {
        ...
        {
        std:
            vector<float> currEmbedding(128);
